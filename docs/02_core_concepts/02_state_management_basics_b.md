...
> **📝 この章の変更について**
>
> このページが、当初の「Flutterの状態管理（State Management）とは何か？」でした。しかし、以下の理由から内容を変更しました:
>
> 1. **まずはFlutterの基本を学ぶべき**: 初心者は、状態管理ライブラリを使う前に、Flutterの基本的な仕組み（`StatefulWidget`や`setState`）をしっかり理解することが重要です。Flutterの標準機能を使うことで、外部ライブラリ不要のため、シンプルで理解しやすく、長期的に安定します。コードは長くなりますが、AIとの協働ではAIが書いてくれるので問題にはなりません。
> 2. **開発初期は変更が多い**: アプリの設計が固まっていない段階でRiverpodを導入すると、仕様変更のたびに状態管理のコードも大きく書き直す必要があり、かえって手間が増えます。
>
> そのため、この章では状態管理の**概念**と**基本的なパターン**に焦点を当て、Riverpodなどのライブラリは「必要になってから学ぶ」というアプローチに修正しました。
